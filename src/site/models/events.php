...
class Sv_Events_LiteModelEvents extends JModelList
{
	...
	protected function getListQuery()
	{
            ...
            if(!empty($month)){
                ...
                $end = date('Y-m-31 23:59:59',strtotime($month));
            }
            ...
            $query->where('((a.datetime_start BETWEEN '.$db->quote($start).' AND '.$db->quote($end).') OR ('
                    . 'a.datetime_end BETWEEN '.$db->quote($start).' AND '.$db->quote($end).'))');
            
            // category
            if(!empty($category)){
                $query->where('a.categories LIKE '.$db->quote('%:"'.$category.'"%'));
            }
            
            

            // Add the list ordering clause.
            $orderCol  = $this->state->get('list.ordering','a.datetime_start');
