 ...
 * @since  1.6
 */
class Sv_Events_LiteViewCategory extends JViewLegacy
{
	protected $state;
	...
	protected function addToolbar()
	{
		...
		{
			$checkedOut = !($this->item->checked_out == 0 || $this->item->checked_out == $user->get('category_id'));
		}
		else
		{
			$checkedOut = false;
		...
		JToolBarHelper::title(JText::_('COM_SV_EVENTS_LITE_TITLE_CATEGORY'), 'category.png');
		...
		if ($this->state->params->get('save_history', 1) && $user->authorise('core.edit')) {
			JToolbarHelper::versions('com_sv_events_lite.category', $this->item->category_id);
		}
